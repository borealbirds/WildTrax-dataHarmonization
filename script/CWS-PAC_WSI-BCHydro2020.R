...
dataset <- "WSI-BCHydro2020"
dataset_code <- "WSI-BCHydro2020"
lu <- "./lookupTables"
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="WSI-BCHydro2020") %>%
...
#       LOAD
#--------------------------------------------------------------
survey <- read_excel(data2020, sheet = "General Survey")
