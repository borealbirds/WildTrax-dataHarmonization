...
dataset <- "WSI-RevelstokeNelson"
dataset_code <- "WSI-RevelstokeNelson"
lu <- "./lookupTables"
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="WSI-RevelstokeNelson") %>%
    select("GSharedDrive location")
  #Download from GoogleDrive
  gd.list <- drive_ls(as.character(pid))
  data1_db <- gd.list %>%
    filter(name =="wsi_5086_org_2012.xls") %>%
    select("id")
  drive_download(as_id(as.character(data1_db)), path = file.path(project_dir, "wsi_5086_org_2012.xls"))
  data2_db <- gd.list %>%
    filter(name =="wsi_5086_org_2013.xls") %>%
    select("id")
  drive_download(as_id(as.character(data2_db)), path = file.path(project_dir, "wsi_5086_org_2013.xls"))
}

data1 <- file.path(project_dir, "wsi_5086_org_2012.xls")
data2 <- file.path(project_dir, "wsi_5086_org_2013.xls")

#--------------------------------------------------------------
#       LOAD
#--------------------------------------------------------------
survey2012 <- read_xls(data1, sheet = "General Survey")
names(survey2012)<-str_replace_all(names(survey2012), c(" " = "_"))

survey2013 <- read_xls(data2, sheet = "General Survey")
names(survey2013)<-str_replace_all(names(survey2013), c(" " = "_"))

survey <- rbind(survey2012, survey2013)
#--------------------------------------------------------------
#
#       TRANSLATE
#
...
pc_location <- spLocation_pj %>%
  ...
  mutate(organization = "CWS-PAC",
         ...
         location = paste0(dataset_code, ":", Study_Area_Name, ":", Block_Label),
         buffer_m = NA,
...
#---VISIT
WTvisit <- c("organization", "location", "visitDate", "snowDepthMeters", "waterDepthMeters", "crew", "bait", "accessMethod", "landFeatures", "comments", 
