# Title: "WSI-WillistonReservoirBCHydro"
# Source dataset is an excel spreadsheet
# Author: "Melina Houle"
# Date: "December 8, 2025"
## Note on translation:
#   - Duration mand distance method were derived from other BC Hydro Peace River project which all used 0m-100m and 0-5min
#----------------------------------------------
#update.packages()
...
dataset <- "WSI-WillistonReservoirBCHydro"
dataset_code <- "WSI-WillistonReservoirBCHydro"
lu <- "./lookupTables"
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="WSI-WillistonReservoirBCHydro") %>%
    select("GSharedDrive location")
  #Download from GoogleDrive
  gd.list <- drive_ls(as.character(pid))

  survey2012 <- gd.list %>%
    filter(name =="5136_WSI_5136_ORG_2012SONGBIRD.XLS") %>%
    select("id")
  drive_download(as_id(as.character(survey2012)), path = file.path(project_dir, "5136_WSI_5136_ORG_2012SONGBIRD.XLS"))
  
  survey2013 <- gd.list %>%
    filter(name =="5136_WSI_5136_ORG_2013SONGBIRDS.XLS") %>%
    select("id")
  drive_download(as_id(as.character(survey2013)), path = file.path(project_dir, "5136_WSI_5136_ORG_2013SONGBIRDS.XLS"))
}

survey2012_tbl <- read_excel(file.path(project_dir, "5136_WSI_5136_ORG_2012SONGBIRD.XLS"), sheet = "General Survey")
survey2013_tbl <- read_excel(file.path(project_dir, "5136_WSI_5136_ORG_2013SONGBIRDS.XLS"), sheet = "General Survey")

common_cols <- intersect(names(survey2012_tbl), names(survey2013_tbl))
survey <- rbind(survey2012_tbl[common_cols], survey2013_tbl[common_cols])
...
survey <- survey %>%
  ...
  dplyr::select(Study_Area_Name, Sample_Station_Label, Date, Time, Surveyor, Species, Count, Easting, Northing)  %>%
...
pc_survey <- s_visit %>% 
  dplyr::rename(site = Sample_Station_Label, 
                ind_count = Count) %>%
...
data_flat <- pc_survey %>% 
  ...
  mutate(species = case_when(Species == "B_MGNW" ~ "MAWA",
                             TRUE ~ species_code),
         scientificname = case_when(Species == "B_MGNW" ~ "Setophaga magnolia",
