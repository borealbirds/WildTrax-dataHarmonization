# Title: "WSI-MackenziePrinceGeorge"
...
dataset <- "WSI-MackenziePrinceGeorge"
dataset_code <- "WSI-MackenziePrinceGeorge"
lu <- "./lookupTables"
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="WSI-MackenziePrinceGeorge") %>%
    select("GSharedDrive location")
  #Download from GoogleDrive
  gd.list <- drive_ls(as.character(pid))
  data1_db <- gd.list %>%
    filter(name =="wsi_4299_dct.xls") %>%
    select("id")
  drive_download(as_id(as.character(data1_db)), path = file.path(project_dir, "wsi_4299_dct.xls"))
}

data <- file.path(project_dir, "wsi_4299_dct.xls")

#--------------------------------------------------------------
#       LOAD
#--------------------------------------------------------------
survey <- read_xls(data, sheet = "Location Behaviour and Sign")
...
pc_location <- spLocation_pj %>%
  ...
  mutate(organization = "CWS-PAC",
         location = paste0(dataset_code, ":", Design_Component_Label),
         buffer_m = NA,
...
############################
s_visit <- survey %>%
  dplyr::select(Design_Component_Label, Date, Time, Surveyor, Species, Count, UTM_Easting, UTM_Northing, Detection_Type, 
                `Detection_Distance_(m)`, Life_Stage, Gender, Time_Interval, `Fly-overs`)  %>%
  dplyr::mutate(location =  paste0(dataset_code, ":", Design_Component_Label),
                ...
                observer = case_when(Surveyor == "Katie Christie" ~ "KC",
                                     Surveyor == "Chris Chutter" ~ "CC",
                                     Surveyor == "Pierre-Paul Baton" ~ "PPB",
                                     Surveyor == "Zoe McDonnell" ~ "ZMD",
                                     Surveyor == "Karl Bachmann" ~ "KB"),
...
unique_observers <- s_visit %>%
  ...
  mutate(
    ...
    observer_id = case_when(Surveyor == "Katie Christie" ~ "KC",
                            Surveyor == "Chris Chutter" ~ "CC",
                            Surveyor == "Pierre-Paul Baton" ~ "PPB",
                            Surveyor == "Zoe McDonnell" ~ "ZMD",
                            Surveyor == "Karl Bachmann" ~ "KB")
...
pc_survey <- s_visit %>% 
  ...
  mutate(organization = organization,
         ...
         distanceband = case_when(`Detection_Distance_(m)` == 1 ~ "0m-50m",
                                  `Detection_Distance_(m)` == 2 ~ "50m-100m",
                                  `Detection_Distance_(m)` == 3 ~ "100m-150m",
                                  `Detection_Distance_(m)` == 4 ~ "150m-200m"),
         durationMethod = "0-3-5min",
         durationinterval = case_when(Time_Interval == "0-3" ~ "0-3min",
                                      Time_Interval == "3-5" ~ "3-5min"),
         isHeard = case_when(Detection_Type == "C" ~ "Yes",
                             Detection_Type == "S" ~ "Yes",
                             Detection_Type == "V, S" ~ "Yes",
                             Detection_Type == "V" ~ "No"),
         isSeen = case_when(Detection_Type == "C" ~ "No",
                            Detection_Type == "S" ~ "No",
                            Detection_Type == "V, S" ~ "Yes",
         ...
         pc_vt = "DNC",
         pc_vt_detail ="DNC",
         age = case_when(Life_Stage == "A" ~ "Adult",
                         Life_Stage == "U" ~ "Unknown"),
         fm = case_when(Gender == "M" ~ "Male",
                        Gender == "U" ~ "Unknown"),
         group = "DNC",
         flyover = `Fly-overs`,
         displaytype = "DNC",
