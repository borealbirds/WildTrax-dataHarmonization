# Title: "Example on translation using WSI-WossRoadside2003"
...
survey <- survey2003 %>%
  mutate(location = paste0(dataset_code, ":", Survey, ":", Point_Count),
         visitDate = ifelse(is.na(Date), "1900-01-01", as.character(as.Date(Date, format = "%Y %m %d"))),
