...
atlas.spid <-merge(data_flat, ATLAS_spId[,c("species_id","common_name", "species_code")], by ="species_id", all.x = TRUE)
...
##test
data_flat_norac[is.na(data_flat_norac$ObservationCount2),]
data_flat_norac$test1 <- as.numeric(data_flat_norac$ObservationCount2) + as.numeric(data_flat_norac$ObservationCount3)
  

data_flat_norac$test2 <- as.numeric(data_flat_norac$ObservationCount) - data_flat_norac$test1

unique(data_flat_norac$test2)
data_flat_norac[as.numeric(data_flat_norac$test2) >0,]

unique(s_data$ObservationDescriptor)
