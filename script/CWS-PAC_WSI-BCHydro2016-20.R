...
WT_spTbl <- read.csv(file.path("./lookupTables/species_codes.csv")) %>%
  dplyr::filter(!species_code %in% c("CORBRA", "PICHUD", "CAJA"))
...
dataset <- "WSI-BCHydro2016-20"
dataset_code <- "WSI-BCHydro2016-20"
lu <- "./lookupTables"
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="WSI-BCHydro2016-20") %>%
    select("GSharedDrive location")
  #Download from GoogleDrive
  gd.list <- drive_ls(as.character(pid))
  data2016_db <- gd.list %>%
    filter(name =="wsi_5800_dct_2016Songbird.xlsm") %>%
    select("id")
  drive_download(as_id(as.character(data2016_db)), path = file.path(project_dir, "wsi_5800_dct_2016Songbird.xlsm"))
  data2017_db <- gd.list %>%
    filter(name =="wsi_5800_dct_2017Songbird.xlsm") %>%
    select("id")
  drive_download(as_id(as.character(data2017_db)), path = file.path(project_dir, "wsi_5800_dct_2017Songbird.xlsm"))
  data2018_db <- gd.list %>%
    filter(name =="wsi_5800_dct_2018Songbird.xlsm") %>%
    select("id")
  drive_download(as_id(as.character(data2018_db)), path = file.path(project_dir, "wsi_5800_dct_2018Songbird.xlsm"))
  data2019_db <- gd.list %>%
    filter(name =="wsi_5800_dct_2019Songbird.xlsm") %>%
    select("id")
  drive_download(as_id(as.character(data2019_db)), path = file.path(project_dir, "wsi_5800_dct_2019Songbird.xlsm"))
  data2020_db <- gd.list %>%
    filter(name =="wsi_5800_dct_2020Songbird.xlsm") %>%
    select("id")
  drive_download(as_id(as.character(data2020_db)), path = file.path(project_dir, "wsi_5800_dct_2020Songbird.xlsm"))
}

data2016 <- file.path(project_dir, "wsi_5800_dct_2016Songbird.xlsm")
data2017 <- file.path(project_dir, "wsi_5800_dct_2017Songbird.xlsm")
data2018 <- file.path(project_dir, "wsi_5800_dct_2018Songbird.xlsm")
data2019 <- file.path(project_dir, "wsi_5800_dct_2019Songbird.xlsm")
data2020 <- file.path(project_dir, "wsi_5800_dct_2020Songbird.xlsm")

#--------------------------------------------------------------
#       LOAD
#--------------------------------------------------------------
survey2016 <- read_xls(data2016, sheet = "General Survey")
names(survey2016)<-str_replace_all(names(survey2016), c(" " = "_"))

survey2017 <- read_xls(data2017, sheet = "General Survey")
names(survey2017)<-str_replace_all(names(survey2017), c(" " = "_"))

survey2018 <- read_xls(data2018, sheet = "General Survey")
names(survey2018)<-str_replace_all(names(survey2018), c(" " = "_"))

survey2019 <- read_xls(data2019, sheet = "General Survey")
names(survey2019)<-str_replace_all(names(survey2019), c(" " = "_"))

survey2020 <- read_xls(data2020, sheet = "General Survey")
names(survey2020)<-str_replace_all(names(survey2020), c(" " = "_"))

survey <- rbind(survey2016, survey2017)
#--------------------------------------------------------------
#
#       TRANSLATE
#
...
pc_location <- spLocation_pj %>%
  dplyr::rename(longitude = x,
                latitude = y) %>%
  mutate(location = paste0(dataset_code, ":", Study_Area_Name, ":", Block_Label))
...
############################
s_visit <- survey %>%
  dplyr::select(Study_Area_Name, Block_Label, Date, Time, Surveyor, Species, Count, Easting, Northing )  %>%
  dplyr::mutate(location =  paste0(dataset_code, ":", Study_Area_Name, ":", Block_Label),
...
data_flat <- pc_survey %>% 
  ...
  mutate(species = case_when(Species  == "B-NAWA" ~ "NAWA", 
                             Species  == "B-HOWR" ~ "UNBI",
                             Species  == "B-NOHA" ~ "NOHA", 
                             TRUE ~ species_code),
