...
dataset_code = "ON-Boreal-Eskers"
source_data <- 'BorealEskers.accdb'
lu <- "./lookupTables"
...
tbls$TABLE_NAME ## allows you to see the table of all the tables in the .accdb 
...
#### LOCATION TABLE ####
############################
pc_location <- sqlFetch(con, "BE_location")
# Transform
...
s_location$site <- s_location$SiteID
...
#### VISIT TABLE ####
############################
pc_visit <- sqlFetch(con, "BE_visit")#Fix column names that have space
...
pc_visit$bait <- "None" # Wildtrax need to have 'None' instead of NA to avoid problems
...
pc_visit$observer <- pc_visit$observer_id
pc_visit$rawObserver <- pc_visit$observer_id
...
pc_visit$pkey_dt<- paste(pc_visit$location, paste0(gsub("-", "", as.character(pc_visit$survey_date)),"_", gsub(":", "", pc_visit$survey_time)), pc_visit$observer_id, sep=":")
...
#### SURVEY TABLE ####
############################
pc_detection <- sqlFetch(con, "BE_detection_grouped")
# get location_name column from BE_visit base on PKEY 2
#extract columns location_name and PKEY2 from pc_visit
pc_visit_sub <- pc_visit %>% select(location_name, PKEY2)
#merge both dataframes by commun ID column PKEY2
pc_detection <- merge(pc_detection, pc_visit_sub, by= "PKEY2", all= TRUE)

s_data <-merge(pc_detection, s_location[,c("location_name","SiteID","Latitude", "Longitude", "lat", "long")], by = "location_name", all.x = TRUE)

data_flat <- merge(s_data, pc_visit, by = c('PKEY2', 'location_name', 'SiteID')) ## by putting all common fields I avoid having duplicate fields that will make cause ERRORS later 
...
survey_tbl <- data_flat[!duplicated(data_flat[,WTsurvey]), WTsurvey] #TODO: problems look for duplicates## CHECK!
