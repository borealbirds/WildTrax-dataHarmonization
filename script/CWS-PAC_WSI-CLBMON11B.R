...
# Note on translation: 
#           - 15 point count use the same name but have different location. Differences vary from 10m to 100m. We decided to keep compute the mean on those. 
#           - 45 station with no obs.were discarded.
#----------------------------------------------
#update.packages()
...
dataset <- "WSI-Arrow Lakes Reservoir 2009-2015"
dataset_code <- "WSI-CLBMON11B"
lu <- "./lookupTables"
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="WSI-CLBMON11B") %>%
    select("GSharedDrive location")
  #Download from GoogleDrive
  gd.list <- drive_ls(as.character(pid))
  data1_db <- gd.list %>%
    filter(name =="wsi_4868_dct_2009-2015Songbirds.xlsm") %>%
    select("id")
  drive_download(as_id(as.character(data1_db)), path = file.path(project_dir, "wsi_4868_dct_2009-2015Songbirds.xlsm"))
}

data1 <- file.path(project_dir, "wsi_4868_dct_2009-2015Songbirds.xlsm")

#--------------------------------------------------------------
#       LOAD
#--------------------------------------------------------------
survey <- read_excel(data1, sheet = "General Survey")
...
# Fix location with duplicate coordinates
pc_location <- pc_location %>%
  ...
  summarize(
    ...
    location_comments = if_else(
      any(abs(latitude - mean(latitude, na.rm = TRUE)) > 1e-6) |
        any(abs(longitude - mean(longitude, na.rm = TRUE)) > 1e-6),
...
s_visit <- survey %>%
  filter(Count >0) %>%
  dplyr::select(Study_Area_Name, Sample_Station_Label, Date, Time, Surveyor, Species, Count, Easting, Northing, Detect_Type, Life_Stage,  Sex)  %>%
...
pc_survey <- s_visit %>% 
  ...
  mutate(organization = organization,
         ...
         pc_vt = case_when(Detect_Type == "CA"  ~ "Call",
                           Detect_Type == "SO"  ~ "Song",
                           Detect_Type == "DR"  ~ "Drumming",
                           TRUE ~ "DNC"),
         pc_vt_detail ="DNC",
         age = "DNC",
         fm = "DNC",
...
data_flat <- pc_survey %>% 
  ...
  mutate(species = case_when(Species == "Aythya"	~	"UDIV",
                             Species ==   "B-GBHE-HE"	~	"GBHE",
                             Species ==  "Laridae"	~	"UNBI",
                             Species ==  "B-NAWA"	~	"NAWA",
                             Species ==  "Hirundinidae"	~	"UNPA",
