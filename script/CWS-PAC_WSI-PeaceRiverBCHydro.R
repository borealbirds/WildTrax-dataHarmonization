...
dataset <- "WSI-PeaceRiverBCHydro"
dataset_code <- "WSI-PeaceRiverBCHydro"
lu <- "./lookupTables"
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="WSI-PeaceRiverBCHydro") %>%
    select("GSharedDrive location")
  #Download from GoogleDrive
  gd.list <- drive_ls(as.character(pid))

  survey <- gd.list %>%
    filter(name =="5137_WSI_5137_ORG_2010POINTCOUNT.XLS") %>%
    select("id")
  drive_download(as_id(as.character(survey)), path = file.path(project_dir, "5137_WSI_5137_ORG_2010POINTCOUNT.XLS"))
}

survey_path <- file.path(project_dir, "5137_WSI_5137_ORG_2010POINTCOUNT.XLS")

#--------------------------------------------------------------
#       LOAD
#--------------------------------------------------------------
survey_tbl <- read_excel(survey_path, sheet = "General Survey")

survey <- survey_tbl %>%
  filter(!if_all(everything(), is.na)) %>%
  rename_with(~ str_replace_all(., " ", "_")) %>%
  dplyr::select(Study_Area_Name, Sample_Station_Label, Date, Time, Surveyor, Species, Count, Easting, Northing, Songign_Type)  %>%
...
pc_survey <- s_visit %>% 
  ...
  mutate(organization = organization,
         ...
         isHeard = case_when(Songign_Type== "Call" ~ "Yes",
                             Songign_Type== "Song" ~ "Yes",
                             Songign_Type =="Visual, Song" ~ "Yes",
                            TRUE ~ "No"),
         isSeen = case_when(Songign_Type =="Visual" ~ "Yes",
                            Songign_Type =="Visual, Song" ~ "Yes",
                            TRUE ~ "No"),
         ...
         #Behaviour
         originalBehaviourData = Songign_Type,
         pc_vt = "DNC",
         pc_vt_detail ="DNC",
         age = "DNC",
         fm = "DNC",
