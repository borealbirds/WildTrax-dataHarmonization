# Title: "WSI-BCHydro2017"
# Source dataset is an excel spreadsheet
# Author: "Melina Houle"
# Date: "November 26, 2025"
## Note on translation:
#   - Delet rows with no obs
#----------------------------------------------
#update.packages()
...
dataset <- "WSI-BCHydro2017"
dataset_code <- "WSI-BCHydro2017"
lu <- "./lookupTables"
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="WSI-BCHydro2017") %>%
...
#       LOAD
#--------------------------------------------------------------
survey <- read_excel(data2017, sheet = "General Survey")
...
s_visit <- survey %>%
  ...
  dplyr::select(Study_Area_Name, Sample_Station_Label, Date, Time, Surveyor, Species, Count, Easting, Northing, Detection_Distance_Class, Comments)  %>%
...
unique_observers <- s_visit %>%
  ...
  mutate(
    observer_name = c("Claudio Bianchini", "Todd Heakes", "Kayla Hatzel"),
    observer_id = c("CB", "TH", "KH")
  )
...
pc_survey <- s_visit %>% 
  ...
  mutate(organization = organization,
         ...
         distanceMethod = "0m-50m-100m",
         distanceband = case_when(Detection_Distance_Class == "0-50" ~ "0m-50m",
                                  Detection_Distance_Class == "51-100" ~ "50m-100m",
                                  Detection_Distance_Class == "0-100" ~ "UNKNOWN",
                                  TRUE ~ "UNKNOWN"),
         durationMethod = "0-5min",
         durationinterval = "0-5min",
         isHeard = case_when(
           grepl("Heard", Comments, ignore.case = TRUE) ~ "Yes",
           grepl("heard", Comments, ignore.case = TRUE) ~ "Yes",
           TRUE ~ "DNC"
         ),
         isSeen = case_when(
           grepl("Seen", Comments, ignore.case = TRUE) ~ "Yes",
           grepl("seen", Comments, ignore.case = TRUE) ~ "Yes",
           TRUE ~ "DNC"
         ),
