...
dataset <- "WSI-BCHydro2016"
dataset_code <- "WSI-BCHydro2016"
lu <- "./lookupTables"
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="WSI-BCHydro2016") %>%
...
#       LOAD
#--------------------------------------------------------------
survey <- read_excel(data2016, sheet = "General Survey")
...
unique_observers <- s_visit %>%
  ...
  mutate(
    observer_name = "Claudio Bianchini",
    observer_id = "CB"
  )
