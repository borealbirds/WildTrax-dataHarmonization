# Title: "WSI-BCHydro2019"
# Source dataset is an excel spreadsheet
# Author: "Melina Houle"
# Date: "November 26, 2025"
## Note on translation:
#   - 7 obs don't have Easting and northing. Dropped
#   - 3 obs don't have birds detected. Dropped
#----------------------------------------------
#update.packages()
...
dataset <- "WSI-BCHydro2019"
dataset_code <- "WSI-BCHydro2019"
lu <- "./lookupTables"
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="WSI-BCHydro2019") %>%
...
#       LOAD
#--------------------------------------------------------------
survey <- read_excel(data2019, sheet = "General Survey")
...
pc_survey <- s_visit %>% 
  ...
  mutate(organization = organization,
         ...
         distanceband = case_when(Detection_Distance_Class == 1 ~ "0m-50m",
                                  Detection_Distance_Class == 2 ~ "50m-100m",
                                  Detection_Distance_Class == 3 ~ "100m-INF",
                                  is.na(Detection_Distance_Class)~ "UNKNOWN"),
