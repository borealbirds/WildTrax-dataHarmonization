# Title: "WSI-Quesnel Forest District 2008-2009"
# Source dataset is an excel spreadsheet
# Author: "Melina Houle"
# Date: "November 4, 2025"
# Note on translation: Flyover were discarded
#----------------------------------------------
#update.packages()
...
dataset <- "WSI-Quesnel Forest District 2008-2009"
dataset_code <- "WSI-QDFA"
lu <- "./lookupTables"
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="WSI-QDFA") %>%
    select("GSharedDrive location")
  #Download from GoogleDrive
  gd.list <- drive_ls(as.character(pid))
  data1_db <- gd.list %>%
    filter(name =="wsi_4558_org_2008.xlsx") %>%
    select("id")
  drive_download(as_id(as.character(data1_db)), path = file.path(project_dir, "wsi_4558_org_2008.xlsx"))
  data2_db <- gd.list %>%
    filter(name =="wsi_4558_org_2009.xlsx") %>%
    select("id")
  drive_download(as_id(as.character(data2_db)), path = file.path(project_dir, "wsi_4558_org_2009.xlsx"))
}

data1 <- file.path(project_dir, "wsi_4558_org_2008.xlsx")
data2 <- file.path(project_dir, "wsi_4558_org_2009.xlsx")

#--------------------------------------------------------------
#       LOAD
#--------------------------------------------------------------
survey2008 <- read_excel(data1, sheet = "Location Behaviour and Sign")
names(survey2008)<-str_replace_all(names(survey2008), c(" " = "_"))

survey2009 <- read_excel(data2, sheet = "Location Behaviour and Sign")
names(survey2009)<-str_replace_all(names(survey2009), c(" " = "_"))

survey <- rbind(survey2008, survey2008)
#--------------------------------------------------------------
#
#       TRANSLATE
#
...
############################
s_visit <- survey %>%
  dplyr::select(Study_Area_Name, Sample_Station_Label, Date, Time, Surveyor, Species, Count, UTM_Easting, UTM_Northing, Detection_Type, Distance_Class,  Interval, Flyover)  %>%
  dplyr::mutate(location =  paste0(dataset_code, ":", Sample_Station_Label),
                visitDate = ifelse(is.na(Date), "1900-01-01", as.character(format(Date, "%Y-%m-%d"))),
                missingvisit = NA,
                rawObserver = Surveyor,
                observer = ifelse(is.na(Surveyor), "obsNA", Surveyor),
...
pc_survey <- s_visit %>% 
  ...
  mutate(organization = organization,
         ...
         distanceMethod = "0m-50m-100m-150m-200m-INF",
         distanceband = case_when(Distance_Class == 1  ~ "0m-50m",
                                  Distance_Class == 2  ~ "50m-100m",
                                  Distance_Class == 3  ~ "100m-150m",
                                  Distance_Class == 4  ~ "150m-200m",
                                  Distance_Class == 5  ~ "200m-INF",
                                  TRUE ~ "UNKNOWN"),
         durationMethod = "0-3-5min",
         durationinterval = case_when(Interval == 1  ~ "0-3min",
                                      Interval == 2  ~ "3-5min",
                                      TRUE ~ "UNKNOWN"),
         isHeard = case_when(Detection_Type == "CA"  ~ "Yes",
                             Detection_Type == "SO"  ~ "Yes",
                             Detection_Type == "VI"  ~ "No",
                             TRUE ~ "UNKNOWN"),
         isSeen = case_when(Detection_Type == "CA"  ~ "No",
                            Detection_Type == "SO"  ~ "No",
                            Detection_Type == "VI"  ~ "Yes",
                            TRUE ~ "UNKNOWN"),
         missingindetections = NA,
         raw_distance_code = Distance_Class,
         raw_duration_code = Interval,
         #Behaviour
         originalBehaviourData = Detection_Type,
         pc_vt = case_when(Detection_Type == "CA"  ~ "Call",
                           Detection_Type == "SO"  ~ "Song",
                           TRUE ~ "UNKNOWN"),
         pc_vt_detail ="DNC",
         age = "DNC",
         fm = "DNC",
         group = "DNC",
         flyover = case_when(Flyover == "FAUX"  ~ "No",
                             Flyover == "VRAI"  ~ "Yes",
                             TRUE ~ "UNKNOWN"),
         displaytype = "DNC",
         nestevidence = "DNC",
...
species_tbl <- pc_survey %>%
  select(Species) %>%
  mutate(BC_Species = paste0("B-",Species)) %>%
  dplyr::distinct(BC_Species) %>%
  left_join(bird_Tbl, by=c("BC_Species"="Species_Code")) %>%
  left_join(WT_spTbl, by=c("Scientific_Name"="scientific_name"))

#list the ones that didn't pass
species_tbl[is.na(species_tbl$BC_Species),]

data_flat <- pc_survey %>% 
  mutate(BC_Species = paste0("B-",Species)) %>%
  left_join(species_tbl, by="BC_Species")  %>%
  mutate(species = case_when(Species  == "WEWP" ~ "WEWP", 
                             Species  == "NRSW" ~ "UNBI", 
                             TRUE ~ species_code),
