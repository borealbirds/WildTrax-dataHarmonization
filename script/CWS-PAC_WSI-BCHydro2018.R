# Title: "WSI-BCHydro2018"
# Source dataset is an excel spreadsheet
# Author: "Melina Houle"
# Date: "November 26, 2025"
## Note on translation:
#   - Sample_Station_Label is written sometimes pc18-055 and PC18-055. it is the same PC. Fix pc to uppercase
#----------------------------------------------
#update.packages()
...
dataset <- "WSI-BCHydro2018"
dataset_code <- "WSI-BCHydro2018"
lu <- "./lookupTables"
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="WSI-BCHydro2018") %>%
...
#       LOAD
#--------------------------------------------------------------
survey <- read_excel(data2018, sheet = "General Survey")
...
#--------------------------------------------------------------
# REPROJECT UTM 10N
spLocation_pj <- vect(survey, geom=c("Easting", "Northing"), crs="epsg:3157")
...
pc_location <- spLocation_pj %>%
  ...
  mutate(organization = "CWS-PAC",
         location = paste0(dataset_code, ":", str_to_upper(Sample_Station_Label)),
         buffer_m = NA,
...
s_visit <- survey %>%
  dplyr::select(Study_Area_Name, Sample_Station_Label, Date, Time, Surveyor, Species, Count, Easting, Northing, Detect_Type, Detection_Distance_Class,
                `Count_0-3_min`, `Count_3-5_min`, `Count_5-10_min`, Comments)  %>%
  dplyr::mutate(location =  paste0(dataset_code, ":",str_to_upper(Sample_Station_Label)),
                visitDate = ifelse(is.na(Date), "1900-01-01", as.character(format(Date, "%Y-%m-%d"))),
                missingvisit = NA,
                rawObserver = Surveyor,
                observer = case_when(Surveyor == "Claudio Bianchini" ~  "CB",
                                     Surveyor == "Todd Heakes" ~  "TH",
                                     Surveyor == "Kerrith McKay" ~  "KMK"),
...
pc_survey <- s_visit %>% 
  ...
  mutate(organization = organization,
         ...
         distanceband = case_when(Detection_Distance_Class == "0-50" ~ "0m-50m",
                                  Detection_Distance_Class == "50-100" ~ "50m-100m",
                                  Detection_Distance_Class == ">100" ~ "100m-INF"),
         durationMethod = "0-3-5-10min",
         durationinterval = case_when(!is.null(`Count_0-3_min`) ~ "0-3min",
                                      !is.null(`Count_3-5_min`) ~ "3-5min",
                                      !is.null(`Count_5-10_min`) ~ "5-10min"), 
         isHeard = case_when(Detect_Type == "CA" ~ "Yes",
                             Detect_Type == "SO" ~ "Yes",
                             Detect_Type == "VI" ~ "No"),
         isSeen = case_when(Detect_Type == "CA" ~ "No",
                            Detect_Type == "SO" ~ "No",
                            Detect_Type == "VI" ~ "Yes"),
         ...
         #Behaviour
         originalBehaviourData = Comments,
         pc_vt = "DNC",
         pc_vt_detail ="DNC",
         age = "DNC",
         fm = "DNC",
...
data_flat <- pc_survey %>% 
  ...
  mutate(species = case_when(Species  == "B-HOWR" ~ "UNBI",
                             Species  == "PASSERIFORMES" ~ "UNPA",
                             Species  == "PICIDAE" ~ "UNWO",
                             Species  == "PARULIDAE" ~ "UNWA",
                             Species  == "HIRUNDINIDAE" ~ "UNSW",
                             Species  == "ANATIDAE" ~ "UNDU",
                             Species  == "Accipitridae" ~ "UNHA",
                             TRUE ~ species_code),
