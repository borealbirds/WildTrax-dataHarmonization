...
#survey attributes
data_flat$visitDate <- format(as.Date(data_flat$Date, format = "%m/%d/%Y"), "%Y-%m-%d") 
...
data_flat$age <- ifelse(data_flat$Observation.Note %in% c("Male","Female"), "Adult", "DNC")
...
data_flat$nestevidence <- ifelse(data_flat$Observation.Note %in% c("At a nest","bird calling from nest", "BWHA calling from active nest"), "Yes", "No")
data_flat$behaviourother <- "DNC"
data_flat$atlas_breeding_code <- "DNC"

data_flat <- data_flat %>%
  mutate(pc_vt = case_when(Detection.Cue =="S" ~ "Song",
                           Detection.Cue =="C" ~ "Calls",
                           Detection.Cue =="O" ~ "NA",
                           Detection.Cue =="NR" ~ "NA",
                           TRUE ~ "DNC" ),
