...
#      - Delete all points where Minute_Detected is after the survey duration method (minutes 15)
...
if (length(list.files(project)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="Okanagan2013") %>%
    select("GSharedDrive location")
  #Download from GoogleDrive
  gd.list <- drive_ls(as.character(pid))
  data1_db <- gd.list %>%
    filter(name =="OKpointcounts2013.xlsx") %>%
    select("id")
  drive_download(as_id(as.character(data1_db)), path = file.path(project_dir, "OKpointcounts2013.xlsx"))
}

data <- file.path(project_dir, "OKpointcounts2013.xlsx")

#--------------------------------------------------------------
#       LOAD
#--------------------------------------------------------------
survey <- read_excel(data, sheet = "Detections New") |>
  ...
  dplyr::filter(!(Minute_Detected ==15))
...
pc_survey <- survey %>% 
  ...
  mutate(organization = organization,
         ...
         isHeard = case_when(Detection_Type == "V/S" ~ "Yes",
                             ...
                             Detection_Type == "V" ~ "No",
                             Detection_Type == "X" ~ "DNC",
                             Detection_Type == "D" ~ "Yes",
                             Detection_Type == "DU" ~ "Yes",
                             Detection_Type == "V/S/C" ~ "Yes",
                             Detection_Type == "V/C/S" ~ "Yes"),
         ...
         pc_vt = case_when(Activity_Code == "TF" ~ "Travelling flight",
                           ...
                           Activity_Code == "D" ~ "Drumming - Woodpecker",
                           Activity_Code == "DU" ~ "Drumming - Ruffed Grouse",
                           TRUE ~ "DNC"),
         pc_vt_detail ="DNC",
         age = case_when(Ageclass == "A" ~ "Adult",
                         Ageclass == "UC" ~ "NA",
                         Ageclass == "JU" ~ "Juvenile",
                         TRUE ~ "UNKNOWN"),
